...
func test_function_definition_executes_when_called():
	...
	assert_eq(BTBehaviour.BTStatus.RUNNING, status)
	status = program.tick(0.5, null, blackboard)
	assert_eq(BTBehaviour.BTStatus.SUCCESS, status)
...
func test_assign_waits_one_second_before_execution():
	...
	assert_eq(BTBehaviour.BTStatus.RUNNING, status)
	assert_eq({}, runtime.snapshot_environment())
	status = program.tick(0.8, null, blackboard)
	assert_eq(BTBehaviour.BTStatus.SUCCESS, status)
